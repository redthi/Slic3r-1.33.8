...
For example in plain state at digit=1 the arrow in segment 1 to 2 points
backwards so if digit=1 then the state changes to reverse for the next
digit.  The direction likewise follows the direction of each segment in the
pattern.
...
L<http://oeis.org/A261180> (etc)

=back

    A261180   direction 0 to 5
    A261185   direction mod 2
    A229214   direction 1,2,3,-1,-2,-3 clockwise
...
Fukuda, Shimizu and Nakamura, "New Gosper Space Filling Curves", on
...
# http://complex-systems.com/pdf/24-4-1.pdf
# http://complex-systems.com/issues/24-4.html
